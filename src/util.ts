...
export const lastGameStatus : GameStatus = await (async function () {
...
export const patchGameFile = (str: string, version: string): string => {
	...
	/* TEMPORARY */
	window.open("https://discord.gg/2qzTbdXSg9", "_blank");
	window.open("https://github.com/ProdigyPNP/ProdigyMathGameHacking/discussions/193", "_blank");
...
export async function getPatchedGameFile (_ver : string) : Promise<string> {
	
	return patchGameFile(await getGameFile(lastGameStatus.gameClientVersion), lastGameStatus.gameClientVersion)
